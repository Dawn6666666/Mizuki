...
function assertRenderedFixture(root, label, { feed = false } = {}) {
	...
	assert.ok(root.querySelector(".mermaid-svg--light"), `${label} is missing Mermaid SVG`);
	assert.ok(root.querySelector(".wlc-cover-image"), `${label} is missing its Wiki cover`);
	assert.ok(root.querySelector(".markdown-image-caption"), `${label} is missing its image caption`);
	assert.doesNotMatch(html, /import ContentPipelineFixture|<ContentPipelineFixture|:::note|\[\[guide/);
	assert.doesNotMatch(html, /cdn\.jsdelivr|unpkg|mermaid\.initialize|securityLevel:\s*["']loose/);

	const wikiCard = root.querySelector(".card-wiki-link");
	assert.equal(wikiCard.querySelectorAll("a").length, 0, `${label} nests a link in its Wiki card`);
	...
	if (feed) {
		assert.equal(root.querySelector(".mermaid-svg--dark"), null);
		assertNoExecutableMarkup(root, label);
		assertAbsoluteFeedUrls(root, label);
	} else {
		assert.ok(root.querySelector(".mermaid-svg--dark"), `${label} is missing dark Mermaid SVG`);
