...
字体模式，并在输出边界统一连线标签的 SVG 文字基线，避免节点、标签或背景框因
浏览器字体与 `tspan` 解析差异而错位。它不会启动浏览器、下载 Chromium，也不调用
外部渲染服务。流程图、时序图、类图、状态图、ER 图、XY 图、Gantt 图和饼图都能
用于正式构建；不支持或无效的语法仍遵循上述 `errorMode`。
