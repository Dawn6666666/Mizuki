...
图片网格、Wiki Link 封面、Mermaid/PlantUML、已有 `figure`，以及带
`data-no-enhance` 的容器不会被重复包装。

### Mermaid

Mermaid 在构建期生成亮色、暗色两份静态 SVG。禁用浏览器 JavaScript 后图表仍
可见；客户端代码只提供缩放、拖动、重置和全屏等渐进增强，不会重新解析源码或
请求公共 CDN。

`src/config/markdownConfig.ts` 中的 `mermaid.errorMode` 可设为：

- `warn`：输出明确的源码降级内容并继续构建；
- `error`：发现无效 Mermaid 时中止构建。

渲染器使用 Mermaid 官方引擎在隔离的 Node.js Worker 中完成解析、布局和 SVG
输出，并由轻量 SVG DOM 提供文字与图形测量。构建期测量与最终 SVG 使用相同的
字体模式，并在轻量 DOM 层将 SVG 文字的 `em` 坐标转换成浏览器等价的用户单位。
标签中的 `<br>` 也会转换成 Mermaid 原生换行，避免多行节点在无 HTML DOM 时丢失
文字。它不会启动浏览器、下载 Chromium，也不调用外部渲染服务。流程图、时序图、
类图、状态图、ER 图、XY 图、Gantt 图和饼图都能用于正式构建；不支持或无效的语法
仍遵循上述 `errorMode`。
...
RSS 与 Atom 保留 Callout 标题、Wiki Link 摘要与封面、MathML、代码组标签和
Mermaid 亮色静态 SVG。代码组会展开，交互脚本和事件属性会移除，文章内相对链接、
public 图片、Astro 构建图片和 `srcset` 全部转换为绝对 URL。Feed HTML 使用显式
标签/属性白名单，RSS 与 Atom XML 都经过严格解析验证。
...
URL、双主题 Mermaid、Wiki 封面和链接分类是否一致。
